...
namespace TPMS.Infrastructure.Services
{
    public class FileStorageService : IFileStorageService
    {
        ...
        public async Task<string> SaveFileAsync(IFormFile file, string ownerType, int ownerId, CancellationToken cancellationToken)
        {
            ...
            string uploadsRoot = Path.Combine(_env.WebRootPath ?? "wwwroot", "uploads");
            string folderPath = Path.Combine(uploadsRoot, ownerType.ToLower(), $"{ownerType}_{ownerId}");
            ...
            // Return relative URL
            return $"/uploads/{ownerType.ToLower()}/{ownerType}_{ownerId}/{file.FileName}";
        }
